...
class DownloadConfig(BaseModel):
    ...
    max_cid: int = Field(
        ..., description="Maximum compound CID to attempt when continuing from last"
    )
    batch_size: int = Field(
        1000, description="Number of successful requests per batch before pausing"
    )
    ...
    request_delay: int = Field(
        3, description="Delay in seconds between requests to PubChem"
    )
...
class ParseConfig(BaseModel):
    ...
    desc_csv: str = Field(
        "descriptions.csv", description="Path to write descriptions table"
    )
